...
class TowersModel(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        user_embeds = tf.expand_dims(self.user_embeds(user), 1)
        movie_embeds = tf.expand_dims(self.movie_embeds(movie), 1)
...
class TowersRecommender(tfrs.models.Model):
    ...
    def compute_loss(self, inputs, training: bool = False) -> tf.Tensor:
        return self.task(labels=tf.expand_dims(inputs['rating'], 1), predictions=self(inputs))
